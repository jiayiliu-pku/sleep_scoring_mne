...
def _merge_annotations(start, stop, description, annotations, current=(), sleep_scoring=False):
    ...
    # Added by Jiayi
    if sleep_scoring is True:
        idx = np.unique(np.append(idx, np.where(annotations.onset == start)))
